...
function detectIntent(text: string, language: 'hi' | 'en'): string {
  ...
  console.log('Detecting intent for text:', lowerText, 'language:', language);
  ...
  for (const service of config.services) {
    if (service.isActive) {
      ...
      console.log('Checking service keywords:', keywords);
      if (keywords.some(keyword => lowerText.includes(keyword.toLowerCase()))) {
        console.log('Found service match, returning booking');
        return 'booking';
      }
    }
  }
    ...
    booking: {
      ...
      en: ['appointment', 'booking', 'book', 'schedule', 'meet', 'photo', 'shoot', 'session', 'booked', 'online']
    },
    tracking: {
  ...
  for (const [intent, keywords] of Object.entries(intents)) {
    console.log(`Checking intent ${intent} with keywords:`, keywords[language]);
    if (keywords[language].some(keyword => lowerText.includes(keyword))) {
      console.log(`Found intent match: ${intent}`);
      return intent;
    }
  }
